from BMM.bmm_main import bmm_track_training_loss
...
def train_backbone(network, scaler, dataset, epoch, config, logger):
    ...
    epoch_losses_train, epoch_probs_train, argmaxXentropy_train, bmm_model, bmm_model_maxLoss, bmm_model_minLoss = \
            bmm_track_training_loss(model, torch.device("cuda" if torch.cuda.is_available() else "cpu") , train_loader=labeled_loader)
...
if __name__ == '__main__':
  ...
  # read config
    ...
    for e in range(1, config['max_iteration']+1):
        auc, acc, cm,pre,epoch_losses_train, epoch_probs_train, argmaxXentropy_train, bmm_model, bmm_model_maxLoss, bmm_model_minLoss = train_backbone(
