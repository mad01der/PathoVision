...
from SPR.spr_utils import get_one_hot
...
def linear(X, Y, num_classes, num_inlier, label):
    ...
    clean_set = []
    noise_set = []
    noise_prob =[0.0 for _ in range(len(label))]
    ...
    for gamma in coefs:
        for i, g in enumerate(gamma):
            if g == 0.0 and i not in clean_set and (selected[label[i]] < num_inlier[label[i]]):
                clean_set.append(i)
                selected[label[i]] += 1
            elif g != 0.0 and i not in noise_set:
                noise_set.append(i)
                noise_prob[i] = g
    ...
    return clean_set, noise_set, noise_prob
...
def spr(config, ep_stats, clean_set):
    idx = np.array(ep_stats['idx'], dtype=int)
    X, label = ep_stats['feature'][idx], ep_stats['label'][idx]-1
    #print("label",label)
    Y = get_one_hot(label, config["num_classes"])
    ...
    if num_classes_sub == config["num_classes"]:
        super2sub = [np.arange(config["num_classes"]).tolist()]
    else:
        if clean_set is None:
            ...
            proto = np.zeros((config["num_classes"], X.shape[1]))
            count = np.zeros(config["num_classes"])
        ...
        candidate = np.arange(config["num_classes"]).tolist()
    ...
    pool = Pool(processes=int(multiprocessing.cpu_count()*0.25))
    ...
    for i, res in enumerate(res_list):
        res = res.get()
        for cl,no,prob in zip(res[0], res[1], res[2]):
            clean_set.append(indexes2full_list[i][cl])
            noise_set.append(indexes2full_list[i][no])
            noise_prob.append(prob)

    clean_set = set(clean_set)
    # noise_set = set(idx) - set(clean_set)
    noise_set = set(noise_set)
