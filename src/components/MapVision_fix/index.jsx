...
export default class MapVision extends Component {
  ...
  setPieChart = async (value1, value2, value3) => {
    const pieData = [
      { name: "no cancer", value: value1, itemStyle: { color: '#40b373' } }, // 绿色
      { name: "cancer", value: value2, itemStyle: { color: '#d4b446' } }, // 蓝色
      { name: "high cancer", value: value3, itemStyle: { color: '#ee8826' } }, // 红色
    ];
    ...
    await this.setState({
      pieOption: {
        ...
        legend: {
          orient: "vertical",
          left: 10,
          data: ["no cancer", "cancer", "high cancer"], // 更新图例数据
        },
        series: [
          {
            name: "信息",
  ...
  handleSelectChange_2 = (value) => {
    ...
    axios
      .post('http://127.0.0.1:5000/last_2', {
  ...
  click1 = () => {
    ...
    axios
      .post('http://127.0.0.1:5000/last2_2', {
  ...
  click2 = () => {
    ...
    console.log("patch_id", this.state.value2)
    axios
      .post('http://127.0.0.1:5000/last3_2', {
        img_id: this.state.choosePatches[this.state.choosePatches.length - 1],
        patch_id: this.state.value2
      })
      .then(response => {
  ...
  click3 = () => {
    ...
    console.log("patch_id", this.state.value2)
    axios
      .post('http://127.0.0.1:5000/last4_2', {
        img_id: this.state.choosePatches[this.state.choosePatches.length - 1],
        patch_id: this.state.value2
      })
      .then(response => {
    ...
    return (
      <>
                ...
                <Row gutter={10}>
                  ...
                  <Col span={5}>
                    <Radio.Group
                      ...
                      buttonStyle="solid"
                    >
                      <Radio.Button value="bmm" selected>Bmm</Radio.Button>
                      <Radio.Button value="spr">PR</Radio.Button>
                    </Radio.Group>
                  </Col>
                </Row>
              </Col>
                  ...
                  <Col span={8}>
                    <div style={{ display: 'flex', flexDirection: 'column', padding: '16px', border: '1px solid #eee', borderRadius: '8px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <Button onClick={this.checkdetails} style={{ backgroundColor: '#1890ff', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: '4px', cursor: 'pointer' }}>details</Button>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '19px', color: '#666' }}>
                        <Text>noise slices number: {this.state.noise_number} </Text>
                        <Text>noise rate: <span style={{ fontWeight: 'bold' }}>{(this.state.noise_number / this.state.total_num_2 * 100).toFixed(2)}%</span></Text>
                        <Text style={{ color: '#40b373' }}>Non-cancerous slices count: {this.state.no_cancer_number}</Text>
                        <Text style={{ color: '#40b373' }}>Non-cancerous slices rate: <span style={{ fontWeight: 'bold' }}>{(this.state.no_cancer_number / this.state.total_num_2 * 100).toFixed(2)}%</span></Text>
                        <Text style={{ color: '#d4b446' }}>Low-cancer slices count: {this.state.cancer_number}</Text>
                        <Text style={{ color: '#d4b446' }}>Low-cancer slices rate: <span style={{ fontWeight: 'bold' }}>{(this.state.cancer_number / this.state.total_num_2 * 100).toFixed(2)}%</span></Text>
                        <Text style={{ color: "#ee8826" }}>High-cancer slices count: {this.state.high_cancer_number}</Text>
                        <Text style={{ color: "#ee8826" }}>High-cancer slices rate: <span style={{ fontWeight: 'bold' }}>{(this.state.high_cancer_number / this.state.total_num_2 * 100).toFixed(2)}%</span></Text>
                      </div>
                    </div>
                  </Col>
                  <Col span={16}> {/* 右侧 Col，占比为 1 */}
                    <Row gutter={[0, 16]} style={{ display: 'flex', flexDirection: 'column' }}>
                      <Col span={24} style={{ flex: 1 }}>
                        <Typography.Title level={4} style={{ fontWeight: 'bold', marginBottom: '5px' }}>The annotation results for this slice</Typography.Title>
                        <Descriptions column={2} bordered>
                          <Descriptions.Item label="labeled count">{this.state.total_num_2 - this.state.do_data.length}</Descriptions.Item>
                          <Descriptions.Item label="Unlabeled count">{this.state.do_data.length}</Descriptions.Item>

                        </Descriptions>
                      </Col>
                      <Col style={{ display: 'flex', alignItems: 'center', marginTop: '35px' }}>
                        <Text style={{ marginRight: '170px', marginLeft: '25px' }}>Click to start labeling task</Text>
                        <Button type="primary" onClick={this.check}>Start</Button>
                      </Col>
                      <Col span={24} style={{ flex: 1,marginTop:'35px' }}>
                        <Typography.Title level={4} style={{ fontWeight: 'bold', marginBottom: '5px' }}>Slice workload statistics</Typography.Title>
                        <Descriptions column={2} bordered>
                          <Descriptions.Item label="labeled count">{this.state.data_total - this.state.count}</Descriptions.Item>
                          <Descriptions.Item label="Unlabeled count">{this.state.count}</Descriptions.Item>
                        </Descriptions>
                      </Col>
                      <Col span={24} style={{ flex: 1 }}>
                        <Text>Under our model, after iterations, the number of labeled slices has automatically increased {this.props.epoch_Data.labeled[this.state.data_epo - 1] -this.props.epoch_Data.labeled[this.state.data_epo - this.state.data_epo] }</Text>
                      </Col>
                      {/* <Col span={24} style={{ flex: 1 }}>
        ...
        <Modal
          ...
          centered
          footer={[
            <Button key="close" onClick={() => this.setState({ isModalVisible: false })}>close</Button>
          ]}
          width={1600}
        >
          <Row>
            <Col span={24}>
              <Descriptions layout="vertical" size="large" column={4} bordered>
                <Descriptions.Item span={2} label="WSI pictures">
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <div style={{ marginLeft: '20px' }}>
                      <Text style={{ marginBottom: '5px', marginRight: '10px' }}>patient Id :20240415110</Text>
                      <Text style={{ marginBottom: '5px' }}></Text>
                    </div>
                  </div>
                </Descriptions.Item>
                <Descriptions.Item span={2} label="WSI image prediction result data" >
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', color: '#666' }}>
                    <Text>noise slices count: {this.state.noise_number} </Text>
                    <Text>noise rate: <span style={{ fontWeight: 'bold' }}>{(this.state.noise_number / this.state.total_num_2 * 100).toFixed(2)}%</span></Text>
                    <Text style={{ color: '#40b373' }}>no cancer count: {this.state.no_cancer_number}</Text>
                    <Text style={{ color: '#40b373' }}>no cancer rate: <span style={{ fontWeight: 'bold' }}>{(this.state.no_cancer_number / this.state.total_num_2 * 100).toFixed(2)}%</span></Text>
                    <Text style={{ color: '#d4b446' }}>low cancer count: {this.state.cancer_number}</Text>
                    <Text style={{ color: '#d4b446' }}>low cancer rate: <span style={{ fontWeight: 'bold' }}>{(this.state.cancer_number / this.state.total_num_2 * 100).toFixed(2)}%</span></Text>
                    <Text style={{ color: "#ee8826" }}>high cancer count: {this.state.high_cancer_number}</Text>
                    <Text style={{ color: "#ee8826" }}>high cancer rate: <span style={{ fontWeight: 'bold' }}>{(this.state.high_cancer_number / this.state.total_num_2 * 100).toFixed(2)}%</span></Text>
                  </div>
                </Descriptions.Item>
                <Descriptions.Item span={4} label="Proportion of images for each label">
                  <ReactECharts
                    style={{ width: 300, height: 300, marginLeft: 600 }}
                    option={this.state.pieOption}
                  />
                </Descriptions.Item>
                <Descriptions.Item span={4} label="Diagnostic recommendations">
                  <div style={{ marginTop: '10px' }}>
                  {this.state.judge == 1 && (
                      <p>The highest proportion of slides shows a high probability of cancer tendency, indicating that the patient's tissue is prone to malignancy. Please take note!</p>
                    )}
                    {this.state.judge == 2 && (
                      <p>The highest proportion of slides indicates a tendency towards cancer, suggesting a possibility of malignancy in the patient's tissue. Please continue to monitor.</p>
                    )}
                    {this.state.judge == 3 && (
                      <p>The highest proportion of slides indicates no cancer tendency, suggesting a lower possibility of malignancy in the patient's tissue. Please take note.</p>
                    )}
                     {this.state.judge == 4 && (
                      <p>The proportion of slides with a high probability of cancer tendency is greater than 20%, indicating that the patient's tissue is prone to malignancy. Please take note!
                      </p>
                    )}
                    {this.state.judge == 5 && (
                      <p>The proportion of slides indicating a cancer tendency is greater than 30%, suggesting a possibility of malignancy in the patient's tissue. Please continue to monitor.</p>
                    )}
                  </div>
        ...
        <Modal
          ...
          footer={[
            <Button key="close" onClick={() => this.setState({ isModalVisible2: false })}>close</Button>
          ]}
            ...
            {this.state.do_data.map((imageUrl, index) => (
              <div key={index} style={{ width: '12.5%', marginBottom: '20px', marginRight: '10px', padding: '0 10px' }}>
                <div style={{ marginBottom: '10px' }}>
                  <img src={process.env.REACT_APP_IMAGE_PATH + "/image_" + this.state.choosePatches[this.state.choosePatches.length - 1] + "/" + imageUrl} style={{ maxWidth: '100%', height: 'auto' }} />
                </div>
        ...
        <Modal
          ...
          footer={[
            <Button key="close" onClick={() => this.setState({ isModalVisible3: false })}>close</Button>
          ]}
          style={{ top: '50%', transform: 'translateY(-50%)' }} // 将Modal居中显示
        >
          <Row gutter={[16, 16]}> {/* 使用 gutter 来设置行之间的间隔 */}
            <Col span={12}> {/* 左侧列，占据总宽度的一半 */}
              <img src={process.env.REACT_APP_IMAGE_PATH + "/image_" + this.state.choosePatches[this.state.choosePatches.length - 1] + "/" + this.state.value1} style={{ maxWidth: '100%', height: 'auto' }} />
            ...
            <Col span={12}> {/* 右侧列，占据总宽度的一半 */}
              <Row gutter={[16, 16]}> {/* 使用 gutter 来设置行之间的间隔 */}
                ...
                <Col span={24}>
                  <Button type="primary" color='green' onClick={this.click1}>No cancer </Button>
                </Col>
                <Col span={24}>
                  <Button type="primary" color='yellow' onClick={this.click2}>cancer </Button>
                </Col>
                <Col span={24}>
                  <Button type="primary" color='red' onClick={this.click3}>high cancer </Button>
                </Col>
              </Row>
            </Col>
          </Row>
