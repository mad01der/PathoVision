...
export default class Pic extends Component {
  ...
  render() {
    ...
    const imagePaths = Array.from({ length: img_len }, (_, index) => ({
      src: img_len > 1  ? process.env.REACT_APP_WSI_PATH + `/${index}.png` : process.env.REACT_APP_WSI_PATH + `/${index}.png`,
      alt: `Image ${index}`,
    }));
    ...
    return (
      <Card bordered={false} hoverable={true}>
        <Col span={15}>
          <Title level={4}> WSI pictures </Title>
        </Col>
        <Col>
            ...
            <div className="image-container">
              {imagePaths.map((image, index) => (
                ...
                <Row>
                  ...
                  <Button onClick={() => this.selectPic.click({ dataIndex: index })}>Select</Button>
                </Row>
              </div>              
              ))}
            </div>
